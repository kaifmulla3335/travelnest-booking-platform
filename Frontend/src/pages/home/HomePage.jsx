...
import { Search, Star, Users, Shield, CheckCircle, Award, HeartHandshake } from 'lucide-react';
...
const CATEGORIES = [
  { label:'Beach',     img: BeachImg,     count:12, filter:'Beach',     color:'from-sky-500/60 to-blue-700/70'      },
  { label:'Mountain',  img: MountainImg,  count:18, filter:'Mountain',  color:'from-slate-500/60 to-slate-800/70'   },
  { label:'Cultural',  img: CulturalImg,  count: 9, filter:'Cultural',  color:'from-amber-500/60 to-orange-700/70'  },
  { label:'Adventure', img: AdventureImg, count:15, filter:'Adventure', color:'from-green-500/60 to-emerald-800/70' },
  { label:'Wildlife',  img: WildlifeImg,  count: 7, filter:'Wildlife',  color:'from-orange-500/60 to-red-700/70'    },
  { label:'Luxury',    img: LuxuryImg,    count: 6, filter:'Luxury',    color:'from-violet-500/60 to-purple-800/70' },
];
...
const WHY_US = [
  {
    ...
    stat: '100%', statLabel: 'Payment Security',
  },
  {
    icon: <Award size={28} />,
    ...
    stat: '500+', statLabel: 'Verified Packages',
  },
  {
    ...
    stat: '< 2min', statLabel: 'Avg Response Time',
  },
  {
    ...
    stat: '50K+', statLabel: 'Happy Travelers',
  },
];

const TESTIMONIALS = [
  { name:'Priya Rawat',    loc:'Mumbai',     initials:'PR', color:'from-sky-400 to-sky-600',       rating:5, text:'TravelNest made our Goa trip absolutely magical. From booking to return — everything was seamless and professional.' },
  { name:'Arjun Kapoor',   loc:'Delhi',      initials:'AK', color:'from-green-400 to-emerald-600', rating:5, text:'Best Ladakh package we could find. The itinerary was perfectly planned and the team was super responsive throughout!' },
  { name:'Sneha Menon',    loc:'Bengaluru',  initials:'SM', color:'from-pink-400 to-rose-500',     rating:5, text:'Booked my Kerala honeymoon through TravelNest — the resort choices, houseboat, everything was five-star treatment.' },
  { name:'Rahul Sharma',   loc:'Pune',       initials:'RS', color:'from-violet-400 to-purple-600', rating:5, text:'The Rajasthan heritage tour was beyond our expectations. Every detail was perfectly organized — truly a royal experience!' },
  { name:'Kavya Iyer',     loc:'Chennai',    initials:'KI', color:'from-teal-400 to-cyan-600',     rating:5, text:'Booked the Andaman trip for our anniversary. The snorkeling experience and beachside resort were absolutely stunning!' },
  { name:'Vikram Singh',   loc:'Jaipur',     initials:'VS', color:'from-amber-400 to-orange-500',  rating:5, text:'Spiti Valley trek was a life-changing experience. TravelNest handled everything perfectly — from permits to camping!' },
  { name:'Meera Nair',     loc:'Kochi',      initials:'MN', color:'from-emerald-400 to-green-600', rating:5, text:'The Kerala backwaters houseboat was pure bliss. Woke up to misty green views every morning. Would book again in a heartbeat!' },
  { name:'Aditya Joshi',   loc:'Hyderabad',  initials:'AJ', color:'from-blue-400 to-indigo-600',   rating:5, text:'Manali snow adventure was thrilling! The ski lessons and Rohtang Pass trip were well-organized. Amazing memories!' },
];
...
const HeroSection = ({ navigate, search, setSearch, handleSearch, siteName }) => (
  <section className="relative min-h-[88vh] sm:min-h-[92vh] flex flex-col items-center justify-center px-4 sm:px-5 text-center overflow-hidden pt-6 sm:pt-0">
    ...
    <div className="relative z-10 max-w-3xl w-full">
      ...
      <div className="inline-flex mb-4 sm:mb-6 cursor-default select-none">
        <div className="relative overflow-hidden bg-white/70 backdrop-blur-md border border-sky-200/80 rounded-full px-3 sm:px-5 py-1.5 sm:py-2 shadow-md shadow-sky-100/40">
          <div className="absolute inset-0 rounded-full overflow-hidden pointer-events-none">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/70 to-transparent -translate-x-full"
          ...
          <span className="relative flex items-center gap-1.5 text-sky-600 text-[11px] sm:text-xs font-semibold">
            <span style={{animation:'starPulse 2s ease-in-out infinite'}}>⭐</span>
      ...
      <h1 className="font-display font-bold text-slate-800 leading-tight mb-3 sm:mb-5
        text-3xl sm:text-5xl md:text-6xl lg:text-7xl">
        Discover Your Next
        <br />
        <span className="relative inline-block">
          <span className="bg-gradient-to-r from-sky-500 via-sky-400 to-blue-600 bg-clip-text text-transparent"
      ...
      <p className="text-slate-500 text-sm sm:text-base md:text-lg mb-6 sm:mb-8 leading-relaxed max-w-xl mx-auto px-2">
        From Himalayan peaks to tropical shores — {siteName} curates unforgettable journeys for every kind of traveler.
      </p>
      ...
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap justify-center
...
const HomePage = () => {
  ...
  return (
    <div>
      {/* ── HERO ── */}
      <HeroSection navigate={navigate} search={search} setSearch={setSearch} handleSearch={handleSearch} siteName={siteName} />

      {/* ── FEATURED PACKAGES ── */}
      <section className="px-4 sm:px-5 py-12 sm:py-16 max-w-7xl mx-auto">
        <div className="text-center mb-8 sm:mb-10">
          <span className="inline-block bg-sky-100 text-sky-600 rounded-full px-4 py-1 text-xs font-bold uppercase tracking-wide mb-3">Featured</span>
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Handpicked Just For You</h2>
          <p className="text-slate-500 text-sm">Explore our most loved travel experiences</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {featured.map(pkg => <PackageCard key={pkg.id} pkg={pkg} />)}
        </div>
        <div className="text-center mt-8">
          <button onClick={() => navigate('/packages')} className="btn-primary px-8 py-3 text-sm">
            View All  Packages →
          </button>
        </div>
      </section>
      ...
      <section className="bg-sky-50/50 py-12 sm:py-16 px-4 sm:px-5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8 sm:mb-10">
            <span className="inline-block bg-sky-100 text-sky-600 rounded-full px-4 py-1 text-xs font-bold uppercase tracking-wide mb-3">Categories</span>
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Travel Your Way</h2>
            <p className="text-slate-500 text-sm">Choose the adventure that suits your style</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {CATEGORIES.map(cat => (
              <button key={cat.label} onClick={() => navigate(`/packages?cat=${cat.filter}`)}
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${cat.color} opacity-0`} />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-end pb-3 sm:pb-4 px-2">
                  <span className="text-white font-display font-bold text-sm sm:text-base leading-tight drop-shadow-lg">{cat.label}</span>
                  <span className="text-white/75 text-xs mt-0.5 font-medium">{cat.count} tours</span>
                </div>
              </button>
            ))}
          </div>
      ...
      <section className="py-14 sm:py-20 px-4 sm:px-5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10 sm:mb-14">
            <span className="inline-block bg-sky-100 text-sky-600 rounded-full px-4 py-1 text-xs font-bold uppercase tracking-wide mb-3">Why Us</span>
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Why {siteName}?</h2>
            <p className="text-slate-500 text-sm">Trusted by 50,000+ travelers across India</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
            {WHY_US.map((item, i) => (
              ...
              >
                ...
                {/* Icon circle */}
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${item.gradient} flex items-center justify-center text-white mb-5 shadow-lg ${item.shadow} group-hover:scale-110 transition-transform duration-300`}>
                  {item.icon}
                </div>

                {/* Title */}
                <h3 className="font-display font-bold text-slate-800 text-base sm:text-lg mb-2">{item.title}</h3>

                {/* Desc */}
                <p className="text-slate-500 text-xs sm:text-sm leading-relaxed mb-5">{item.desc}</p>

                {/* Bottom stat */}
                <div className={`flex items-center gap-2 pt-4 border-t border-slate-100`}>
                  <span className={`font-display font-bold text-lg bg-gradient-to-r ${item.gradient} bg-clip-text text-transparent`}>
                    {item.stat}
                  </span>
                ...
                <div className={`absolute top-4 right-4 w-2 h-2 rounded-full bg-gradient-to-br ${item.gradient} opacity-40 group-hover:opacity-80 transition-opacity`} />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-sky-50/50 py-14 sm:py-20 overflow-hidden">
        <div className="text-center mb-8 sm:mb-12 px-4">
          <span className="inline-block bg-sky-100 text-sky-600 rounded-full px-4 py-1 text-xs font-bold uppercase tracking-wide mb-3">Testimonials</span>
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-slate-800 mb-2">What Travelers Say</h2>
          <p className="text-slate-500 text-sm">Real stories from real adventurers</p>
        </div>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-5">
        <div className="max-w-2xl mx-auto relative overflow-hidden rounded-3xl"
          style={{background:'linear-gradient(135deg, #0c1e3c 0%, #0a2a4a 50%, #0d1f3c 100%)'}}>
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {[['15%','20%'],['75%','15%'],['85%','60%'],['20%','75%'],['55%','85%'],['40%','30%'],['65%','45%']].map(([l,t],i)=>(
              <div key={i} className="absolute w-0.5 h-0.5 bg-white/40 rounded-full" style={{left:l,top:t}} />
            ))}
          </div>

          <div className="relative z-10 p-8 sm:p-12 text-center">
            {/* Animated emoji */}
            <div className="text-4xl sm:text-5xl mb-5 inline-block"
            ...
            <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-white mb-3">
              {isLoggedIn ? 'Your Next Adventure Awaits!' : 'Ready to Explore the World?'}
            </h2>
            ...
            <button onClick={handleCTA}
              ...
              {isLoggedIn ? 'Explore All Packages →' : "Get Started — It's Free ✈️"}
            </button>
